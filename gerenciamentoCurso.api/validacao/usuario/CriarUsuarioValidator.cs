...
{
    public class CriarUsuarioValidator : BaseAbstractValidator<CriarUsuarioCommand>
    {
       
        public CriarUsuarioValidator(IServiceProvider serviceProvider) : base(serviceProvider)
        {
            ...
            RuleFor(usuario => usuario)
                .Must( usuario => !service.AnyUsuarioAsyncByEmailCpf(usuario.Email, usuario.Cpf).Result).WithMessage("Cpf ou email cadastrado");
